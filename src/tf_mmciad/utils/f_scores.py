...
@tf.keras.utils.register_keras_serializable(package="mmCIAD")
class FBetaScore(tf.keras.metrics.Metric):
    ...
    Args:
        ...
        average: Type of averaging to be performed on data.
            ...
            `weighted` and `report`. Default value is None.
        ...
        focus: (Optional) Class index to focus reporting on. Works
            only together with the 'report' `average` option.
    Returns:
        F-Beta Score: float.
    Raises:
        ValueError: If the `average` has values other than
        `[None, 'micro', 'macro', 'weighted', 'report']`.
    ...
    `average` parameter behavior:
        ...
        report: Metrics are computed for each class
            and only the score of the class in focus
            is returned.
    Usage:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if average not in (None, "micro", "macro", "weighted", "report"):
            raise ValueError(
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        self.y_true = tf.reshape(tf.cast(y_true, self.dtype), [-1, self.num_classes])
        self.y_pred = tf.reshape(tf.cast(y_pred, self.dtype), [-1, self.num_classes])
        ...
        self.true_positives.assign_add(_weighted_sum(self.y_pred * self.y_true, sample_weight))
        self.false_positives.assign_add(
            _weighted_sum(self.y_pred * (1 - self.y_true), sample_weight)
        )
        self.false_negatives.assign_add(
            _weighted_sum((1 - self.y_pred) * self.y_true, sample_weight)
        )
        self.weights_intermediate.assign_add(_weighted_sum(self.y_true, sample_weight))
    ...
    def result(self):
        ...
        elif self.average == "report":
            f1_score = f1_score[self.focus]
...
class F1Score(FBetaScore):
    ...
    def __init__(
        ...
        super().__init__(num_classes, average, 1.0, threshold, name=name, dtype=dtype, focus=focus)

...
class DataGenerator(Sequence):
    ...
    def __init__(self, path, colorvec, means, stds, list_IDs, batch_size=32, dim=(208,208), n_channels=3,
                 n_classes=10, shuffle=True, augmenter=True):
        'Initialization'
        self.dim = dim
        self.path = path
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        y_class = [np.zeros((*self.dim, 1), dtype=np.uint8) for _ in range(self.batch_size)]
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            for cls_ in range(self.n_classes):
                ...
                y_class[i] += np.expand_dims(
                    np.logical_and.reduce(y[i,] == color,axis=-1)*cls_, axis=-1).astype('uint8')
